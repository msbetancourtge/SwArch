...
import { Plus, Clock, Timer, Eye, EyeOff } from 'lucide-react';
import { 
  Table, 
  TableHeader, 
  TableRow, 
...
import { type Product, type CreateProductDTO } from '@/lib/types';
import { useAuth } from '@/contexts/AuthContext';

// Categorías alineadas estrictamente con los Enums del Backend
...
export const AdminProductsPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingProduct) {
        ...
        console.log('Producto actualizado:', { id: editingProduct.id, ...formData });
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-50/50 min-h-screen">
      ...
      {/* Filtros ajustados a la Base de Datos */}
      <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <Label htmlFor="search" className="text-sm font-bold text-slate-700">Buscar</Label>
            <Input
              id="search"
          ...
          <div className="space-y-2">
            <Label htmlFor="category" className="text-sm font-bold text-slate-700">Categoría</Label>
            <select
              id="category"
      ...
      {/* Tabla Limpia (Solo campos de la API) */}
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="text-center py-20 text-slate-400 font-medium">Cargando catálogo...</div>
        ) : (
          <Table>
                  ...
                  <TableCell>
                    <div className="w-14 h-14 rounded-xl overflow-hidden border border-slate-100 shadow-sm">
                      <img
                        ...
                        onError={(e) => { (e.target as HTMLImageElement).src = 'https://placehold.co/100x100?text=Plato'; }}
                      />
                    </div>
                  </TableCell>
                  <TableCell>
  ...
  <DialogContent className="max-w-md rounded-2xl overflow-hidden">
    <DialogHeader>
      ...
      <DialogDescription>Completa los campos requeridos para el menú.</DialogDescription>
    </DialogHeader>
    ...
    <form onSubmit={handleSubmit} className="space-y-4 pt-2">
      ...
      <Dialog open={!!deleteConfirm} onOpenChange={() => setDeleteConfirm(null)}>
        <DialogContent className="max-w-xs rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-center font-bold">¿Eliminar producto?</DialogTitle>
            <DialogDescription className="text-center">Esta acción borrará el plato permanentemente del menú.</DialogDescription>
          </DialogHeader>
          <DialogFooter className="flex flex-col gap-2 pt-2">
            <Button variant="destructive" onClick={() => deleteConfirm && handleDelete(deleteConfirm)} className="w-full">
              Sí, eliminar
            </Button>
            <Button variant="ghost" onClick={() => setDeleteConfirm(null)} className="w-full">Cancelar</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
