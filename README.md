...
#### Applied Architectural Tactic

**Detect Attacks** — The WAF filter decodes request content (up to two URL-decode passes to defeat percent-encoding evasion) and matches it against signature lists for SQLi, XSS, and Path Traversal. Any match triggers an immediate HTTP 403 rejection before JWT validation or downstream routing runs.

#### Applied Architectural Pattern

**WAF (Web Application Firewall)** — A `GlobalFilter` at the highest filter priority intercepts every incoming request at the API Gateway edge, screening paths, query parameters, and key headers against known attack signatures. Downstream microservices are never exposed to malicious traffic.
...
**Encrypt Data** — TLS 1.2 / 1.3 encrypts all traffic between external clients and the API Gateway, preventing eavesdropping and man-in-the-middle attacks. Modern cipher suites are enforced and deprecated protocols (SSLv3, TLS 1.0/1.1) are disabled.

#### Applied Architectural Pattern

**Secure Channel** — TLS is terminated at the sole public entry point (API Gateway). All internal `appnet` traffic stays on the private Docker bridge network, keeping certificate management confined to a single component.
...
### 6.1 Pattern Description and Reliability Tactics

The **Cluster Pattern** groups multiple compute nodes under a unified management layer that acts as a single logical system for clients. Kubernetes implements this pattern through three main abstractions:

| Abstraction | Function |
|---|---|
| **Pod** | Minimum deployment unit; encapsulates one or more containers |
| **Deployment** | Declares the desired state (replicas, image, update strategy); the Controller Manager continuously reconciles it |
| **Service** | Stable, single endpoint that load-balances traffic across active Pods |

The **reliability tactics** it supports:

- **Fault detection** — Liveness and Readiness Probes detect unhealthy pods and automatically remove them from the traffic pool.
- **Redundant spare** — Multiple replicas ensure that the failure of one instance does not interrupt the service.
- **Load balancing** — kube-proxy distributes requests across healthy replicas, maximising resource usage and avoiding hot spots.
- **Auto-healing** — The Controller Manager continuously reconciles the real state with the desired state: if a Pod dies, a new one is created.
- **Rolling updates** — Image updates are applied by replacing Pods one by one with zero downtime.

### 6.2 Cluster Type Implemented

The **APIGateway** deployment uses an **Active/Active** cluster:

> All replicas (2 by default, scalable to N) serve traffic simultaneously. The Kubernetes Service acts as a load balancer, distributing each incoming HTTP/WebSocket request to any available Pod in a round-robin fashion.

**Rationale:** The APIGateway is completely *stateless* — it only validates the JWT and routes the request to the corresponding microservice. It holds no session or in-memory state between requests. This means any replica can handle any request without coordination, which is the necessary and sufficient condition for an Active/Active cluster.

### 6.3 Redundancy Pattern

#### Description

**Active Redundancy (Hot Spare)** corresponds to the **Redundant Spare** tactic within the *Recover from Faults > Preparation and Repair* group of Len Bass's reliability tactics catalog.

**Definition:** All instances in the protection group (active and spare) **receive and process the same requests in parallel** at all times. State remains consistent because all replicas share the same source of truth (the PostgreSQL database). When one instance fails, the others continue operating without interruption and without needing a synchronisation process, since they were already fully up to date.

**Selected component: AuthService**

AuthService is the most critical microservice on the platform: no authenticated operation can run if this service fails. Because it is a **stateless in-memory service** (all persistence lives in PostgreSQL), it is the ideal candidate for Active Redundancy: multiple replicas can process login and registration requests completely independently.

### Quality Scenario:

| Element | Description |
|---|---|
| **Source** | Internal infrastructure failure (JVM process) |
| **Stimulus** | An AuthService Pod terminates unexpectedly due to an OutOfMemoryError during the lunch peak (12:00–14:00) |
| **Artifact** | AuthService — authentication and authorisation microservice (port 8081) |
| **Environment** | Normal operation under peak load: ~500 concurrent login requests distributed across 2 active replicas |
| **Response** | The liveness probe detects the failed pod in ≤ 30 s; kube-proxy removes that endpoint from the pool; the surviving replica absorbs 100% of the traffic immediately; the Controller Manager schedules a replacement pod; the PodDisruptionBudget guarantees that both replicas never go down simultaneously |
| **Response Measure** | Zero authentication requests lost (the spare was already actively processing traffic); detection time ≤ 30 s; time to restore 2 replicas ≤ 90 s; user-perceptible downtime: 0 ms |

### 6.4 Cold Spare (NotificationService)

#### Description

**Passive Redundancy (Cold Spare)** corresponds to the **Redundant Spare** tactic within the *Recover from Faults > Preparation and Repair* group of Len Bass's reliability tactics catalog.

**Definition:** Only **one instance** of the service is active and processing requests at any time. There is no pre-warmed spare instance. When the active instance fails, the orchestrator (Kubernetes) creates a new instance (the "cold spare" boots up), which must go through a full startup cycle before it can handle traffic. During this startup period the service is **unavailable** — but because RabbitMQ acts as a message buffer, **no event is lost**.

**Selected component: NotificationService**

NotificationService is **non-critical for business operations**: orders, payments, and reservations work normally without notifications. This makes it the ideal candidate for Cold Spare: a downtime window of ~60–90 s is accepted in exchange for lower resource consumption (one instance instead of two).

#### Hot Spare vs Cold Spare Comparison

| Aspect | Hot Spare (AuthService) | Cold Spare (NotificationService) |
|---|---|---|
| **Active replicas** | 2 (both process traffic) | 1 (no pre-warmed spare) |
| **CPU/RAM consumption** | 2× (double resources) | 1× (minimal resources) |
| **Downtime on failure** | 0 ms | ~60–90 s (spare startup) |
| **PDB** | `minAvailable: 1` | `maxUnavailable: 1` |
| **Pod anti-affinity** | Yes (spread across nodes) | No (single replica) |
| **Message loss** | None | None (RabbitMQ retains them) |
| **State synchronisation** | Continuous (shared DB) | Not required |
| **Ideal use case** | Critical services (auth, payments) | Latency-tolerant services |
...
|---|---|
| **Source** | Internal infrastructure failure (JVM process) |
| **Stimulus** | The only active NotificationService Pod terminates unexpectedly due to an OutOfMemoryError during the lunch peak (12:00–14:00) |
| **Artifact** | NotificationService — notifications and SSE streaming microservice (port 8087) |
| **Environment** | Normal operation: 1 active instance, 0 pre-warmed spare instances, ~200 order/reservation events per hour queued in RabbitMQ |
| **Response** | The liveness probe detects the failed pod in ≤ 45 s; the Controller Manager schedules a new pod (the cold spare boots up); RabbitMQ retains all messages in `notification.order.queue` and `notification.reservation.queue` without loss; the new pod starts, connects to PostgreSQL and RabbitMQ, and processes the accumulated messages |
| **Response Measure** | **RTO ≤ 90 s** (pod startup time); **RPO = 0** (no messages lost); during RTO, REST requests return HTTP 503; SSE connections drop but clients reconnect automatically and receive the accumulated notifications |
...
curl -X POST http://localhost:8080/order \
  ...
  -d '{
    ...
    "items": [
      {"itemName": "Burger", "notes": "no lettuce"},
      {"itemName": "Burger", "notes": "with everything"}
    ]
  }'
```
...
curl -X POST http://localhost:8080/checkout \
  ...
  -d '{
    ...
    "items": [
      {"menuItemId": "abc123", "productName": "Burger", "quantity": 2, "unitPrice": 15000}
    ],
    "paymentMethod": "CARD"
  }'
```
